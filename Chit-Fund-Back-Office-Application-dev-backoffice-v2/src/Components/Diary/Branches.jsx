    ...
    <Box>
      <TableContainerWithBorder>
        <Table>
          <TableHead>
            <VoucherTableHeadRow>
              {["Sr. No.", "Name of branches", "Amount"].map((header) => (
          ...
          <TableBody>
            {tableData.map((row, idx) => (
              <VoucherTableBodyRow key={idx}>
                ...
                <TableBodyCell>{row.nameOfBranch || "-"}</TableBodyCell>
                <TableBodyCell>
                  <RupeeFieldContainerInTable>
                    <LuIndianRupee
                      style={{
            ...
            <VoucherTableBodyRow sx={{ backgroundColor: "#F3F2F5" }}>
              ...
              <TableBodyCell style={{ fontWeight: 500, textAlign: "right" }}>
                <RupeeFieldContainerInTable>
                  ...
                  />
                  {finalTotalData.credit.toLocaleString("en-IN")}
                </RupeeFieldContainerInTable>
              </TableBodyCell>
            </VoucherTableBodyRow>
          </TableBody>
        ...
        <Table>
          ...
          <TableBody>
            {tableData.map((row, idx) => (
              <VoucherTableBodyRow key={idx}>
                <TableBodyCell>{row.srNo}</TableBodyCell>
                <TableBodyCell>{row.branchId || "-"}</TableBodyCell>
                <TableBodyCell>{row.branch || "-"}</TableBodyCell>
                <TableBodyCell>{row.onAcof || "-"}</TableBodyCell>
                <TableBodyCell>
                  <RupeeFieldContainerInTable>
                    <LuIndianRupee
                      style={{
              ...
              <TableBodyCell sx={{ fontWeight: 500 }}>
                <RupeeFieldContainerInTable>
                  ...
                  <LuIndianRupee
                    style={{ marginLeft: 4, marginRight: 2, fontSize: "13px" }}
                  />
                  {finalTotalData.debit.toLocaleString("en-IN")}
                </RupeeFieldContainerInTable>
              </TableBodyCell>
            </VoucherTableBodyRow>
          </TableBody>
