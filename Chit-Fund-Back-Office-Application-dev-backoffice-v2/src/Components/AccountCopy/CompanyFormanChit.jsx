...
const AccountCopyCompanyFormanChit = () => {
  const [selectedNode, setSelectedNode] = useState("");
  const [selectedForman, setSelectedForman] = useState("");
  ...
  const fetchData = async () => {
    try {
      // Replace with actual API call for company/forman chit data
      // const response = await getAccountCopyCompanyFormanChit("company-forman-chit", 1, 10);
      // setData(response.data);
      console.log("Fetching company/forman chit data...");
    } catch (error) {
  ...
  return (
    <Box sx={{ flexGrow: 1, backgroundColor: "#F5F6FF" }}>
      <TopContainerForAllPages>
        <TopModuleName>Company/Forman chit</TopModuleName>
      </TopContainerForAllPages>
        ...
        >
          ...
          >
            <Grid container spacing={2} sx={{ mb: 2 }}>
              ...
              >
                <InputLabelLead>Select node</InputLabelLead>
              ...
              <Grid
                ...
                sx={{ alignItems: "center" }}
              >
                <InputLabelLead>Select forman</InputLabelLead>
                <FormControl fullWidth size="small">
                  <LeadDataInput
                    select
                    fullWidth
                    value={selectedForman}
                    onChange={(e) => setSelectedForman(e.target.value)}
                    placeholder="Select Forman"
                    variant="outlined"
                    size="small"
                    SelectProps={{
                      displayEmpty: true,
                      renderValue: (selected) => {
                        ...
                        const selectedOption = formanOptions?.find(
                          (option) => option.id === selected
                        );
                        return selectedOption ? selectedOption.name : selected;
                      },
            ...
            <TableBody>
              {tableData.map((row, idx) => (
                <VoucherTableBodyRow key={idx}>
                  ...
                  <TableBodyCell>{row.ticket || "-"}</TableBodyCell>
...
export default AccountCopyCompanyFormanChit;
