      ...
      <Table sx={{ width: "125%", overflowX: "auto" }}>
        <TableHead>
          <VoucherTableHeadRow>
            ...
            <TableHeadCell>Default inter collected</TableHeadCell>
            <TableHeadCell>Dt ARR realised</TableHeadCell>
            <TableHeadCell>ARR amount collected</TableHeadCell>
        ...
        <TableBody>
          {tableData.map((row, idx) => (
            <VoucherTableBodyRow key={idx}>
              ...
              <TableBodyCell>
                <RupeeFieldContainerInTable>
                  ₹{row.defaultInterCollected?.toLocaleString("en-IN") || "0"}
                </RupeeFieldContainerInTable>
              </TableBodyCell>
              <TableBodyCell>{row.dtArrRealised || "-"}</TableBodyCell>
              <TableBodyCell>
                <RupeeFieldContainerInTable>
                  ₹{row.arrAmountCollected?.toLocaleString("en-IN") || "0"}
                </RupeeFieldContainerInTable>
              </TableBodyCell>
              <TableBodyCell>{row.report || "-"}</TableBodyCell>
            </VoucherTableBodyRow>
          ))}
          ...
          <VoucherTableBodyRow sx={{ backgroundColor: "#F3F2F5" }}>
            ...
            <TableBodyCell>
              Total: ₹
              {(finalTotalData?.defaultInterCollected ?? 0).toLocaleString(
                "en-IN"
              )}
            </TableBodyCell>
            <TableBodyCell>
              Total: ₹
              {(finalTotalData?.dtArrRealised ?? 0).toLocaleString("en-IN")}
            </TableBodyCell>
            <TableBodyCell>
              Total: ₹
              {(finalTotalData?.arrAmountCollected ?? 0).toLocaleString(
                "en-IN"
              )}
            </TableBodyCell>
            <TableBodyCell />
...
export default BillCollectorArrear;
